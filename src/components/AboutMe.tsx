...
export function AboutMe() {
  ...
  return (
    <div
      ref={ref}
      className="relative flex flex-col lg:flex-row items-center justify-center h-auto w-full overflow-hidden px-6 lg:px-20 py-20 bg-gradient-to-br from-gray-900 via-black to-gray-900"
    >
      ...
      {/* Left: Profile Image with Hover & Parallax */}
      <motion.div
      ...
      >
        <motion.h1
          ...
          className="mb-4 text-5xl sm:text-6xl font-extrabold bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 bg-clip-text text-transparent"
        >
          About Me
        </motion.h1>
        ...
        <motion.ul
          variants={itemVariants}
          className="space-y-2 mb-6 text-gray-400"
        >
        ...
        <motion.a
          ...
          className="bg-indigo-500 hover:bg-indigo-600 text-white px-6 py-3 rounded-lg shadow-lg transition-transform transform hover:scale-105"
        >
          Resume
        </motion.a>
      </motion.div>
      ...
      <motion.div
        className="absolute top-10 left-10 w-32 h-32 bg-indigo-500 rounded-full opacity-20"
        animate={{ scale: [1, 1.2, 1], rotate: [0, 45, 0] }}
        transition={{ duration: 6, repeat: Infinity }}
      />
      <motion.div
        className="absolute bottom-20 right-20 w-48 h-48 bg-pink-500 rounded-full opacity-20"
...
// Skill Icon Component with Hover Animation
function SkillIcon({ icon, name }: { icon: React.ReactNode; name: string }) {
  return (
    ...
    >
      {icon}
      <span className="text-sm text-gray-400 mt-1">{name}</span>
    </motion.div>
  );
}
